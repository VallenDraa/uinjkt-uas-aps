...
const AdminSingleOrder = () => {
	...
	useEffect(() => {
		...
		const fetchOrderProducts = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/order-product/${params?.id}`,
			);
	...
	const deleteOrder = async () => {
		...
		fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/order-product/${order?.id}`,
			requestOptions,
		).then(response => {
			fetch(
							...
							<Image
								src={
									product?.product?.mainImage
										? `${process.env.NEXT_PUBLIC_MEDIA_URL}${product?.product?.mainImage}`
